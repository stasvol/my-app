...
const initialState = {
  users: [
    // {id: 1, photoUrl:"https://i.pinimg.com/originals/53/08/
    // 1c/53081c48b54b7be2805a0b2ad5470735.jpg",
    // followed: true, name: 'Andre', status: "I'm  Cool"},
    // {id: 2, photoUrl:"https://i.pinimg.com/originals/b4/98/f9/
    // b498f91f653cd9ed231209b12fac64c7.jpg",
...
const SetUserReducer = (state = initialState, action) => {
  switch (action.type) {
    case FOLLOW:
      return {
        ...state,
        users: changeDataRedux(state.users, action.userId, 'id', {
          followed: true,
        }),
      };
    case UNFOLLOW:
...
export const currentPageSetAcCr = currentPageSet => ({
  type: CURRENT_PAGE_SET,
  currentPageSet,
});

export const settingUserTotalCountAcCr = countUsersSet => ({
  type: SETTING_USER_TOTAL_COUNT,
  countUsersSet,
});
...
export const setLoadDisableButAcCr = (isLoad, userId) => ({
  type: SET_DISABLE_BUTTON,
  isLoad,
  userId,
});

export const newSetStatus = (status, newPutStatus) => ({
  type: NEW_SET_STATUS,
  status,
  newPutStatus,
});
...
export const showPhoto = file => async dispatch => {
  const response = await newApiStatus.showPutPhoto(file);
  // .then(response => {
  if (response.data.resultCode === 0) {
...
export const saveContacts = prof => async (dispatch, getState) => {
  ...
  const response = await newApiStatus.savePutContacts(prof);
  // .then(response => {
  if (response.data.resultCode === 0) {
