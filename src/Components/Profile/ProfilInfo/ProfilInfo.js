...
const ProfInfo = ({profile, savePhoto, editProfile, status, isOwner, updateStatus, ...props}) => {
    const {editMode, onPhotoChange, onSubmit, handleGoToEditMode} = useProfilInfo(profile, savePhoto, editProfile)
        ...
        <div>
            <div>
                <img className={classes.imgCont} alt={'image'}
                     src={'https://sites.google.com/site/prirodanasevseegooglgfgf/_' +
            ...
            <div className={classes.contact}>
                ...
                <div>
                    <img src={profile.photos.small || kot} className={classes.image} alt={'image'}/>

                    {isOwner && <input type={"file"} onChange={onPhotoChange}/>}

                </div>
                {editMode
                    ? <ProfDataForm {...props} onSubmit={onSubmit} initialValues={profile}/>
